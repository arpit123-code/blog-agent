...
class Task(BaseModel):
    ...
    bullets:List[str]=Field(
        ...
        min_length=3,
        max_length=5,
        description="A list of bullet points that should be covered in this section.",
    )
    target_words:int=Field(
        ...,
        description="The target number of words for this section (120-550).",
    )
...
- Extract/normalize published_date as ISO (YYYY-MM-DD) if you can infer it from title/snippet.
  If you can't infer a date reliably, set published_date=null (do NOT guess).
- Keep snippets short.
- Deduplicate by URL.
"""
...
def research_node(state:State)->dict:
    ...
    if mode == "open_book":
        ...
        for e in evidence:
            d = _iso_to_date(e.published_date)
            if d and d >= cutoff:
                fresh.append(e)
        evidence = fresh
...
CRITICAL:
- You MUST return VALID JSON that EXACTLY matches the Plan schema.
- NEVER return empty tasks.
- ALWAYS include 5–9 tasks.
- If unsure, STILL generate tasks (do NOT return empty).
...
FAILSAFE:
If you cannot follow instructions, STILL return a valid Plan with placeholder content.
        ...
        [
            ...
            HumanMessage(
                content=(
                    f"Topic: {state['topic']}\n"
                    f"Mode: {mode}\n"
                    f"As-of : {state['as_of']} (recency {state['recency_days']} days)\n"
